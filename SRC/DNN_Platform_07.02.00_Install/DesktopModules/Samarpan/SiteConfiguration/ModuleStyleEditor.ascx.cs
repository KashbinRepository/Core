...
namespace MICROS.Modules.MyCentralSiteConfiguration
{
    ...
    partial class ModuleStyleEditor : PortalModuleBase
    {
        ...
        private string GetTempDefaultXmlHelperFile()
        {
            var objPortalController = new PortalController();
            PortalInfo objPortal = objPortalController.GetPortal(PortalId);
            string tempDefaultXml = string.Format("{0}\\Portals\\_default\\Skins\\{1}\\TempDefaultXml.xml", Globals.ApplicationMapPath, objPortal.PortalName);

            return tempDefaultXml;
        }
        ...
        {
            string defaultXxmlPath = string.Format("{0}\\Portals\\_default\\Skins\\WebOrderingES\\DefaultES.xml", Globals.ApplicationMapPath);
            string tmpDefaultXmlPath = GetTempDefaultXmlHelperFile();
            string customValueXml = GetCustomXMLHelperFile();
            if (File.Exists(defaultXxmlPath))
            {
                File.Copy(defaultXxmlPath, tmpDefaultXmlPath, true);
                UpdateOriginalXML(tmpDefaultXmlPath, customValueXml);
            }
            CopyXMLFile(tmpDefaultXmlPath, customValueXml);
            if(File.Exists(tmpDefaultXmlPath))
            {
                File.Delete(tmpDefaultXmlPath);
            }
        }
        ...
        private XmlNode GetCurrentModuleXmlNode()
        {
            ...
            string CustomXML = GetCustomXMLHelperFile();

            XmlDocument xmlDoc = new XmlDocument();
            xmlDoc.Load(CustomXML);
        ...
        private XmlDocument UpdateHelperFile()
        {
            ...
            return UpdateOriginalXML(CurrentEditNode, CustomValueXML);
        }
