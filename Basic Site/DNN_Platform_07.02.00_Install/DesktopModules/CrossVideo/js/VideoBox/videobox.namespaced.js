var Videobox = {
	...
	init: function (options) {
		...
		this.options = Object.extend({
			...
			flvplayer: CROSS_ULTRAFLASHPLAYER_FLV_PLAYER_FILE //'swf/flvplayer.swf'
		}, options || {});
		...
		this.center = new Moo.Element('div').setProperty('id', 'lbCenter').setStyles({width: this.options.initialWidth+'px', height: this.options.initialHeight+'px', marginLeft: '-'+(this.options.initialWidth/2)+'px', display: 'none'}).injectInside(document.body);

		this.bottomContainer = new Moo.Element('div').setProperty('id', 'lbBottomContainer').setStyle('display', 'none').injectInside(document.body);
...
//		else if (sLinkHref.match(/\.flv/i)|| sLinkHref.match(/\.f4v/i)) {
		else if (sLinkHref.match(/\.flv/i) || sLinkHref.match(/\.f4v/i) || sLinkHref.match(/\.mp4/i)) {
		this.flash = true;
		 //2010/11/12 º”»Îautostart=true
